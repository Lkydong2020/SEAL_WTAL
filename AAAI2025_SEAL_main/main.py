...
from config_SEAL import options
from Dataset_SEAL import VideoDataset
...
def train(args, dataset, model, device):
    ...
    if os.path.exists(os.path.join(checkpoint_dir, 'best_model_SEAL.pkl')):
       os.remove(os.path.join(checkpoint_dir, 'best_model_SEAL.pkl'))
    ...
    for epoch in tqdm(range(1, args.max_epoch+1)):
        ...
        if epoch % args.interval == 0:
            ...
            if cond:
                torch.save(model.state_dict(), os.path.join(checkpoint_dir, 'best_model_SEAL.pkl'))
                max_map = dmap

    model.load_state_dict(torch.load(os.path.join(checkpoint_dir, 'best_model_SEAL.pkl')))
...
if __name__ == '__main__':
    ...
    model = SEAL(args, appearance_descriptors, motion_descriptors).to(device) # (args, appearance_descriptors, motion_descriptors)
